...
def test_001_table_group():
    ...
    """
    _sql_list = list(pg_table_sql.values())
    ...
    high_risk_resources.add_high_risk_source_permission_yes_database(_connection_permission, _connection, _database, _pgUser, _table_group, _table)
...
def test_003_only_apply_one_permission():
    ...
    high_risk_resources.delete_all_high_risk_source_permission(_connection)
    high_risk_resources.add_high_risk_source_permission_yes_database(["Insert"], _connection, "SALESPDB", _pgUser, _table_group, _table)
    for i in _sql_list:
