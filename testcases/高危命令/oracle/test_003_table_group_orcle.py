...
username = "gaowei"
_v = permission_data["Oracle"]
# ##########测试数据###########
# _connection = create_db_connection("oracle")
_connection = "xyz_test6666_oracle"
...
def test_001_table_group():
    ...
    :return:
    """
    _sql_list = list(oracle_tableGroup_sql.values())
    # 清除环境，没有添加任何高危权限设置时，执行对应权限SQL语句，可以执行，不会有高危提示
    high_risk_resources.delete_all_high_risk_source_permission(_connection)
    for i in _sql_list:
        res = db_sql.run_sql_oracle(_connection, i, _oracleUser, username)
        assert "成功" in res
    # 通用配置中，关闭高危复核方式，添加高危权限设置，执行对应权限SQL语句，有高危提示，没有复核提示
    high_risk_resources.add_high_risk_source_permission_no_database(_tableGroup_permission, _connection, _oracleUser, "表")
...
def test_002_username_apply_permission():
    ...
    for i in _sql_list:
        res = db_sql.run_sql_oracle(_connection, i, "", username)
...
def test_003_only_apply_one_permission():
    ...
    _sql_list = list(oracle_connection_sql.values())
    high_risk_resources.delete_all_high_risk_source_permission(_connection)
    high_risk_resources.add_high_risk_source_permission_no_database(["Insert"], _connection, _oracleUser, "表")
    for i in _sql_list:
