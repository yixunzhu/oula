...
username = "gaowei"
_v = permission_data["SQLServer"]
# ##########测试数据###########
_connection = "xyz_test6666_SqlServer"
_database = "SALESPDB"
_sqlserverUser = "GAOWEI_SCHEMA"
_table_group = "表"
_table = "BIAO3333"
_column = "a"
...
def test_001_table_group():
    ...
    """
    _sql_list = list(sqlserver_tableGroup_sql.values())
    # 清除环境，没有添加任何高危权限设置时，执行对应权限SQL语句，可以执行，不会有高危提示
    high_risk_resources.delete_all_high_risk_source_permission(_connection)
    for i in _sql_list:
        res = db_sql.run_sql_sqlserver(_connection, i, _database, username)
        assert "成功" in res
    # 通用配置中，关闭高危复核方式，添加高危权限设置，执行对应权限SQL语句，有高危提示，没有复核提示
    high_risk_resources.add_high_risk_source_permission_yes_database(_connection_permission, _connection, _database, _sqlserverUser, _table_group)
...
def test_002_username_apply_permission():
    ...
    :return:
    """
    _sql_list = list(sqlserver_database_sql.values())
...
def test_003_only_apply_one_permission():
    ...
    high_risk_resources.delete_all_high_risk_source_permission(_connection)
    high_risk_resources.add_high_risk_source_permission_yes_database(["Insert"], _connection, "SALESPDB", _sqlserverUser, _table_group)
    for i in _sql_list:
        if "insert" in i.lower():
            res = db_sql.run_sql_sqlserver(_connection, i, username)
            assert "语句中存在对高危资源操作" not in res
            res = db_sql.run_sql_sqlserver(_connection, i, _database, username)
