...
def test_002_oracle_schema_preview(fixture_delete_all_db_connection):
    """
    添加表后的SQL预览
    :param fixture_delete_all_db_connection:
    :return:
    ...
    fixture_delete_all_db_connection()
    _db_connection = db_sql.create_db_connection("oracle")
    res = sdt_click.sdt_right_click_preview(_db_connection)
    assert '"resCode":10000,"resMsg":"成功"' in res
    assert '"data":{"sql":"CREATE' in res


@pytest.mark.skip("")
def test_003_import(fixture_delete_all_db_connection):
    """
    右键文本导入
    :param fixture_delete_all_db_connection:
    :return:
