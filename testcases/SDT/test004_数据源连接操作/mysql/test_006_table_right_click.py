...
def test_001_table_right_click(fixture_delete_all_db_connection):
    ...
    print(_data)
    _list = ['SQL', '刷新', '复制', '导出', '导出提权', '打开表', '文本导入', '查看表结构', '添加表', '编辑表', '脱敏提权', '设计表', '重命名']
...
def test_004_right_click_table_rename(fixture_delete_all_db_connection):
    ...
    db_sql.run_sql_mysql(_db_connection, _sql)
    res = sdt_click.sdt_right_click_table_rename(_db_connection, "", "TB_SCHEMA", "TB_TABLE{}".format(_random),
                                                 "TB_TABLE")
    sdt_click.sdt_right_click_table_rename(_db_connection, "", "TB_SCHEMA", "TB_TABLE", "TB_TABLE{}".format(_random))
    assert '"resCode":10000,"resMsg":"成功"' in res
