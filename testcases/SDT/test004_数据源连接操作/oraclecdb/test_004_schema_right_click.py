...
def test_001_oraclecdb_schema_invalid_object(fixture_delete_all_db_connection):
    """
    编译无效对象
...
def test_002_oraclecdb_schema_look(fixture_delete_all_db_connection):
    """
    编译无效对象-错误查看
    :param fixture_delete_all_db_connection:
    :return:
...
def test_003_export(fixture_delete_all_db_connection):
    ...
    payload = {
        ...
        "nodeName": "TB_SCHEMA",
        "nodeType": "oracleUser",
        "nodePath": "/root/{}/SALESPDB/TB_SCHEMA".format(connection_id)
    }
    payload = json.dumps(payload)
    headers = {
        'Cookie': '{}'.format(public.get_cookie()),
